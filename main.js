...
class Tree {
    ...
    */
    insertNode(parentNode, newNode, direction = "left") {
        if (direction == "left") {
    ...
    initalizeTwoChildren(parentNode) {
        let leftChild = new NodeObject(0);
        let rightChild = new NodeObject(1);
...
let recordedValues = [];
let branches = [];
function visitChildren(node, depth) {
    if (node == null)
        return 1;
    //Adding combination
...
let currentTree = generateTree(TREE_LENGTH);
generateBranches();
outputData();
function outputData() {
...
(_a = document.getElementById("number-of-statements")) === null || _a === void 0 ? void 0 : _a.addEventListener("change", function () {
